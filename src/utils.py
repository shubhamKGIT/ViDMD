...
def read_pickle_dump(filepath):
    "reading the pickle file, arbitrarty data"
    with open(filepath, "rb") as f:
        return pickle.load(f)

def read_pickle_dumped_array(filepath):
...
def get_2D_mesh(image = None, shape = None):
    if image is None:
        img_shape = shape
    else:
        img_shape = image.shape
    print(f"for 3D/ surf plot, image has shape: {img_shape}")
    X = np.linspace(0, img_shape[1], img_shape[1])
    Y = np.linspace(0, img_shape[0], img_shape[0])
